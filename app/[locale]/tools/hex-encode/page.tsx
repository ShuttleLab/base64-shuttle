...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  if (locale === "zh") {
    return {
      title: "十六进制编码器（在线）– 免费 Hex 编码与转换工具",
      description: "一键把文本转换为十六进制格式。免费在线 Hex 编码器，在浏览器中把字符串转换为其十六进制字节表示。",
      alternates: {
        canonical: `/${locale}/tools/hex-encode`,
  ...
  if (locale === "zh") {
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-16">
        <h1 className="text-3xl font-bold tracking-tight mb-4">{t("hexEncode.title")}</h1>
        <p className="text-lg text-muted-foreground mb-10">{t("hexEncode.subtitle")}</p>

        <Card className="border-primary/30 bg-primary/5">
          <CardContent className="flex flex-col items-center gap-4 py-10 text-center">
            <p className="text-base font-medium">在首页打开 Hex 编码工具，立即开始转换。</p>
            <Link
              href="/zh/#tool"
        ...
        <p className="text-center text-sm text-muted-foreground mt-10">
          <Link href="/tools/hex-encode" className="underline hover:text-foreground transition-colors">
            {t("fullGuide")}
          </Link>
        </p>
      </div>
