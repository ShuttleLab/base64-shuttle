...
- `npm run build` = `next build && node scripts/postbuild.mjs`. `next build` compiles, type-checks, and static-exports to `out/`; the **postbuild step is mandatory** — the export is not deployable without it (see Build Pipeline below).
- There is **no test suite** configured.

## Build Pipeline (`scripts/postbuild.mjs`)

`next build` with `output: "export"` + `localePrefix: "as-needed"` emits the default English locale under `out/en/`, but every canonical/hreflang/sitemap URL declares English at the web root. The postbuild script reconciles the output with what the metadata promises:

1. Promotes `out/en/*` → `out/` (so `/` serves English) and deletes `out/en`.
2. Patches `<html lang="en">` → `<html lang="zh-CN">` in all `out/zh/**/*.html`.
3. Generates `out/sw.js` — a service worker with a precache list derived from the *actual* built routes, plus `_next/static` cache-first, navigation network-first, and stale-while-revalidate for assets.

Because the SW and root-URL layout are produced here (not by Next.js), debugging routing/caching means reading this script, not just the `app/` tree. `next.config.ts` also sets `trailingSlash: true`.
...
- Tailwind CSS v4 + shadcn/ui (primitives from `@base-ui/react`)
- Geist + Geist Mono fonts
- Theme: System/Light/Dark mode **plus accent-color palettes**, both via `theme-sync.tsx` (`useTheme`, `PALETTES`) and surfaced by `components/palette-picker.tsx`
- `service-worker-register.tsx` registers `/sw.js` (production only); the SW itself is generated by the postbuild step
